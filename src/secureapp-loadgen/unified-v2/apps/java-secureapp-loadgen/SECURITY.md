...
See `k8s/loadgen/loadgen-entries.sh` (java entries) for the full endpoint → attack-class mapping.

## Scanner suppression policy

For each CVE reported against this manifest (`pom.xml`):

- **Do not open a fix PR.**
- **Do not run `mvn versions:use-latest-versions` or equivalent.**
- Suppress in the scanner with reason: **"Intentionally vulnerable demo target — see `apps/java-secureapp-loadgen/SECURITY.md`."**
...
Questions about whether a specific CVE is intentional here: check `k8s/loadgen/loadgen-entries.sh` first, then ask the SecureApp demo owner before upgrading.
