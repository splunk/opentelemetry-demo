...
The app runs the shared loadgen (`k8s/loadgen/loadgen.sh`, copied into the image at `/app/loadgen.sh`) that hits vulnerability endpoints on independent schedules controlled by a flagd feature flag.
...
The shared loadgen (`k8s/loadgen/loadgen.sh` / `loadgen-entries.sh`) fires vulnerability endpoints on independent schedules, controlled by the `secureappAttack` flagd feature flag:

| Pace | Attack frequency |
|------|-----------------|
| `minimal` (default) | Each vuln every ~3-12 hours |
| `targeted` | SQLi + Log4Shell every ~10-15 min, others minimal |
| `max` | All attacks every ~3-15 min |

Controlled via `Unified-v2/k8s/values.env` (`DATAGEN_MODE=loop` enables in-container loadgen). Shipping traffic is driven by inbound Astronomy Shop requests when `SHIPPING_API=true`, not by a separate loadgen timer.
