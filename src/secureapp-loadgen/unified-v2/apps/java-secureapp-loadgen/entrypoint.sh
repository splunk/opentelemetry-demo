...
LOADGEN_ENABLED="${LOADGEN_ENABLED:-true}"
if [ "${LOADGEN_ENABLED}" = "true" ] || [ "${LOADGEN_ENABLED}" = "1" ]; then
  echo "Starting shared loadgen (LOADGEN_ENABLED=${LOADGEN_ENABLED}, LOADGEN_LANGUAGE=${LOADGEN_LANGUAGE:-java})..."
  export LOADGEN_LANGUAGE="${LOADGEN_LANGUAGE:-java}"
  export ATTACK_HOST="${ATTACK_HOST:-localhost:8080}"
  /app/loadgen.sh &
  LOADGEN_PID=$!
  trap "kill $LOADGEN_PID 2>/dev/null; exit" EXIT
else
  echo "LOADGEN_ENABLED=${LOADGEN_ENABLED} — skipping built-in loadgen."
fi
