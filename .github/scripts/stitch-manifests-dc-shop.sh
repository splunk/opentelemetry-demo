...
# Script to stitch together Kubernetes manifests for DC-SHOP version
# Based on stitch-manifests.sh but with modifications:
# - INCLUDES shop-dc-* services (shop-dc-shim, shop-dc-shim-db, shop-dc-loadgenerator)
...
#
# Usage: ./stitch-manifests-dc-shop.sh [registry_env]
...
echo "Creating DC-SHOP manifest for version: $VERSION"
...
echo "Found ${#SERVICES[@]} services for DC-SHOP manifest (includes ALL services)"
...
OUTPUT_FILE="$OUTPUT_DIR/splunk-astronomy-shop-dc-shop-${VERSION}.yaml"
...
cat > "$OUTPUT_FILE" << EOF
# Splunk Astronomy Shop Kubernetes Manifest - DC-SHOP Version
# Version: $VERSION
# Generated on: $(date -u +"%Y-%m-%dT%H:%M:%SZ")
#
# This is the DC-SHOP configuration variant:
# - INCLUDES shop-dc-* services (shop-dc-shim, shop-dc-shim-db, shop-dc-loadgenerator)
# - Uses modified flagd-config with paymentFailure defaultVariant = "off"
#
...
echo ""
echo "=========================================="
echo "DC-SHOP Manifest stitching complete!"
echo "=========================================="
echo "Version: $VERSION"
echo "Output file: $OUTPUT_FILE"
echo "Services included: $FOUND (ALL services with manifest: true)"
