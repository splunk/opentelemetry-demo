...
# Script to stitch together Kubernetes manifests for PAYMENT ERROR version
# Based on stitch-manifests.sh but with modifications:
# - Excludes shop-dc-* services
# - Uses modified flagd-config with paymentFailure defaultVariant = "50%"
#
...
#
# Usage: ./stitch-manifests-payment-error.sh [registry_env]
...
echo "Creating PAYMENT ERROR manifest for version: $VERSION"
...
# Filter out shop-dc-* services for Payment Error version
SERVICES=()
EXCLUDED=()
...
echo "Found ${#SERVICES[@]} services for PAYMENT ERROR manifest (${#EXCLUDED[@]} shop-dc services excluded)"
...
OUTPUT_FILE="$OUTPUT_DIR/splunk-astronomy-shop-payment-error-${VERSION}.yaml"
...
cat > "$OUTPUT_FILE" << EOF
# Splunk Astronomy Shop Kubernetes Manifest - PAYMENT ERROR Version
# Version: $VERSION
# Generated on: $(date -u +"%Y-%m-%dT%H:%M:%SZ")
#
# This is the PAYMENT ERROR configuration variant:
...
echo ""
echo "=========================================="
echo "PAYMENT ERROR Manifest stitching complete!"
echo "=========================================="
